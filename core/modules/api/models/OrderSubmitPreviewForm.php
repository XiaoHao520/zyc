...
//            $express_price = PostageRules::getExpressPrice($this->store_id, $address['province_id']);
    ...
    private function getDataByGoodsInfo($goods_info, $store)
    {
        ...
        $goods_cat=GoodsCat::findOne(['goods_id'=>$goods_info->goods_id]);
        $goods_cat=GoodsCat::find()->andWhere(['goods_id'=>$goods_info->goods_id,'is_delete'=>0])->all();
        $goods_cat=$goods_cat[0];
       $cat=null;
        if($goods_cat){
            $cat=Cat::findOne(['id'=>$goods_cat['cat_id']]);
        }
...
//                    $resIntegral['forehead'] = sprintf("%.2f", ($store->integral * $goodsPrice));
        ...
        return [
            ...
            'data' => [
                ...
                'integral' => $resIntegral,
                'goods_card_list'=>$goods_card_list,
                'cat_buy_method'=>$cat['buy_method'],
                'goods_cat'=>$goods_cat['cat_id']
            ],
        ];
    }
