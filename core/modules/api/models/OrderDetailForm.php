...
//            ->where(['od.order_id' => $order->id, 'od.is_delete' => 0])->asArray()->all();
        $goods_list = OrderDetail::find()->alias('od')->leftJoin(['g' => Goods::tableName()], 'od.goods_id=g.id')
            ->select('g.id as goods_id,g.time AS goods_time,g.attr As attrs,od.id AS order_detail_id,g.name,od.attr,od.num,od.total_price,od.pic')
            ->where(['od.order_id' => $order->id, 'od.is_delete' => 0])->asArray()->all();
        $num = 0;
    ...
    public function changeDate(){
        ...
        if($order){
           $res= Order::updateAll(array('date'=>$this->date),'id=:id',array(":id"=>$this->order_id));
           return $res;
        }
        return 0;
    }
    public function changeTime(){
        ...
        if($order){
            $res= Order::updateAll(array('time'=>$this->time),'id=:id',array(":id"=>$this->order_id));
            return $res;
        }
        return 0;
    }
    public function changeRoute(){
        ...
        if($order){
            $res= OrderDetail::updateAll(array('attr'=>$this->route),'id=:id',array(":id"=>$this->order_id));
            return $res;
        }
        return 0;
    }
