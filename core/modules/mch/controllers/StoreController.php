...
use app\modules\mch\models\DockForm;
...
class StoreController extends Controller
{
     ...
     * 码头列表
     */
    public function actionDock()
    {

         $dock_list=Dock::find()->where(['store_id'=>$this->store->id,'is_delete'=>0])->orderBy('addtime DESC')->all();
        return $this->render('dock', [
            'dock_list' => $dock_list,
        ]);
    ...
    public function actionDockEdit($id = null)
    {
        $dock = Dock::findOne(['id' => $id]);
        if (!$dock) {
            $dock = new Dock();
        }
        $form = new DockForm();
...
//                'province_list' => District::findAll(['level' => 'province']),
    ...
    {
        $shop = Shop::findOne(['id' => $id, 'store_id' => $this->store->id, 'is_delete' => 0]);
        $docks=Dock::findAll(['store_id'=>$this->store->id]);
        if (!$shop) {
            $shop = new Shop();
        }
        if (\Yii::$app->request->isPost) {
